...
class CanvasViewCache extends Object
{
	...
	void getCanvasPositionAndSize()
	{
		if (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_2)
		{
			...
			System.err.println("Canvas pos = (" + awtCanvasX + ", " + awtCanvasY + "), size = " + awtCanvasWidth + "x" + awtCanvasHeight);
	...
	synchronized void snapshot(boolean computeFrustum)
	{
		...
		int dirtyIndex = computeFrustum ? Canvas3D.RENDER_BIN_DIRTY_IDX : Canvas3D.RENDERER_DIRTY_IDX;
	...
	private void doComputeDerivedData(boolean currentFlag, CanvasViewCache cvc, BoundingBox frustumBBox, boolean doInfinite)
	{
		...
		// Issue 109 : determine the the correct index to use -- either the
		// Renderer or RenderBin
		int dirtyIndex = (frustumBBox != null) ? Canvas3D.RENDER_BIN_DIRTY_IDX : Canvas3D.RENDERER_DIRTY_IDX;
		int scrvcDirtyMask;
		...
		if (!canvas.manualRendering && (vprNotDirty) && (cvcDirtyMask == 0) && (scrvcDirtyMask == 0) && (viewCache.vcDirtyMask == 0)
		...
		if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_1))
		{
			...
			if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_2))
			{
				...
				{
					System.err.println("leftFrustumPlanes " + i + " is " + leftFrustumPlanes[i]);
				}

				for (i = 0; i < rightFrustumPlanes.length; i++)
				{
					System.err.println("rightFrustumPlanes " + i + " is " + rightFrustumPlanes[i]);
				}
			}
		}
	...
	private void computeCanvasInfo()
	{
		...
		Rectangle screenBounds = new Rectangle((int) canvas.graphicsConfiguration.getBounds().getX(),
				(int) canvas.graphicsConfiguration.getBounds().getY(), (int) canvas.graphicsConfiguration.getBounds().getWidth(),
				(int) canvas.graphicsConfiguration.getBounds().getHeight());
		...
		physicalWindowXLeft = metersPerPixelX * (double) canvasX;
		physicalWindowYBottom = metersPerPixelY * (double) (screenHeight - canvasHeight - canvasY);
		...
		physicalWindowCenter.x = physicalWindowXLeft + physicalWindowWidth / 2.0;
		physicalWindowCenter.y = physicalWindowYBottom + physicalWindowHeight / 2.0;
		...
		if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_2))
		{
			...
			System.err.println("Window LL corner (in plate coordinates): " + "(" + physicalWindowXLeft + "," + physicalWindowYBottom + ")");

			System.err.println("Window size (in plate coordinates): " + "(" + physicalWindowWidth + "," + physicalWindowHeight + ")");

			System.err.println("Window center (in plate coordinates): " + physicalWindowCenter);
		...
		if (!viewCache.compatibilityModeEnable && viewCache.viewPolicy == View.HMD_VIEW)
		{
			if (!useStereo)
			{
				...
				case View.LEFT_EYE_VIEW:
					headTrackerToLeftImagePlate.set(screenViewCache.headTrackerToLeftImagePlate);
					break;

				case View.RIGHT_EYE_VIEW:
					headTrackerToLeftImagePlate.set(screenViewCache.headTrackerToRightImagePlate);
					break;
				}
			}
			else
			{
				...
				headTrackerToRightImagePlate.set(screenViewCache.headTrackerToRightImagePlate);
			}
	...
	private void computeViewPlatformScale()
	{
		...
		if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_2))
		{
			System.err.println("viewCache.windowResizePolicy = " + viewCache.windowResizePolicy);
	...
	private void cacheEyePosFixedField()
	{
		...
		rightEyeInImagePlate.y = leftEyeInImagePlate.y = physicalWindowCenter.y;
		...
		if (!useStereo)
		{
			...
			case View.LEFT_EYE_VIEW:
				leftEyeInImagePlate.x = physicalWindowCenter.x + viewCache.leftEyePosInHead.x;
				break;

			case View.RIGHT_EYE_VIEW:
				leftEyeInImagePlate.x = physicalWindowCenter.x + viewCache.rightEyePosInHead.x;
				break;
			}
		...
		else
		{
			...
			rightEyeInImagePlate.x = physicalWindowCenter.x + viewCache.rightEyePosInHead.x;
		}
		...
		//
		rightEyeInImagePlate.z = leftEyeInImagePlate.z = physicalWindowWidth / (2.0 * Math.tan(viewCache.fieldOfView / 2.0));
		...
		if (this.viewCache.view.soundScheduler != null)
			this.viewCache.view.soundScheduler.setListenerFlag(SoundScheduler.EYE_POSITIONS_CHANGED);
	}
	...
	private void cacheEyePosWindowRelative()
	{
		...
		rightEyeInImagePlate.z = leftEyeInImagePlate.z = leftManualEyeInImagePlate.z;
		...
		else
		{
			...
			rightEyeInImagePlate.z = rightManualEyeInImagePlate.z;
		}
	...
	private void computeTrackedEyePosition()
	{
		...
		if (viewCache.viewPolicy != View.HMD_VIEW)
		{
			...
			headToLeftImagePlate.transform(viewCache.leftEyePosInHead, leftTrackedEyeInImagePlate);

			headToLeftImagePlate.transform(viewCache.rightEyePosInHead, rightTrackedEyeInImagePlate);
		}
		else
		{
			if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_2))
			...
			headToLeftImagePlate.mul(headTrackerToLeftImagePlate, viewCache.headToHeadTracker);

			headToLeftImagePlate.transform(viewCache.leftEyePosInHead, leftTrackedEyeInImagePlate);

			if (useStereo)
			{
				headToRightImagePlate.mul(headTrackerToRightImagePlate, viewCache.headToHeadTracker);

				headToRightImagePlate.transform(viewCache.rightEyePosInHead, rightTrackedEyeInImagePlate);
			}
			else
			{ // HMD_VIEW with no stereo.
				headToLeftImagePlate.transform(viewCache.rightEyePosInHead, rightTrackedEyeInImagePlate);
			}
	...
	private void cacheEyePosition()
	{
		if (viewCache.compatibilityModeEnable)
		{
			...
			cacheEyePosScreenRelative(leftManualEyeInImagePlate, rightManualEyeInImagePlate);
		}
		else if (viewCache.getDoHeadTracking())
		{
			computeTrackedEyePosition();
			cacheEyePosScreenRelative(leftTrackedEyeInImagePlate, rightTrackedEyeInImagePlate);
		}
		else
		{
			...
			case View.RELATIVE_TO_COEXISTENCE:
				cacheEyePosCoexistenceRelative(viewCache.leftManualEyeInCoexistence, viewCache.rightManualEyeInCoexistence);
				break;
			}
		}
		...
		if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_1))
		{
			System.err.println("leftEyeInImagePlate = " + leftEyeInImagePlate);
			System.err.println("rightEyeInImagePlate = " + rightEyeInImagePlate);
			System.err.println("centerEyeInImagePlate = " + centerEyeInImagePlate);
			System.err.println("nominalEyeOffset = " + nominalEyeOffset);
			System.err.println();
		}
	}
	...
	private void computePlateToVworld()
	{
		...
		// these new values can be sent to the AudioDevice
		if (this.viewCache.view.soundScheduler != null)
			this.viewCache.view.soundScheduler.setListenerFlag(SoundScheduler.IMAGE_PLATE_TO_VWORLD_CHANGED);
	}
	...
	private void computeHeadToVworld()
	{
		...
		// these new values can be sent to the AudioDevice
		if (this.viewCache.view.soundScheduler != null)
			this.viewCache.view.soundScheduler.setListenerFlag(SoundScheduler.HEAD_TO_VWORLD_CHANGED);
	}
		...
		if (viewCache.viewPolicy != View.HMD_VIEW)
		{
			...
			case View.NOMINAL_SCREEN:
				...
				case View.NOMINAL_FEET:
					tVec1.set(0.0, -viewCache.nominalEyeHeightFromGround, nominalEyeOffset);
					tMat2.set(tVec1);
					break;
				}
			...
			case View.NOMINAL_HEAD:
				...
				case View.NOMINAL_HEAD:
					...
					break;
				case View.NOMINAL_FEET:
					tVec1.set(0.0, -viewCache.nominalEyeHeightFromGround, 0.0);
					tMat2.set(tVec1);
					break;
				}
				break;
	...
	private void computeCoexistenceCenter()
	{
		...
		if ((!viewCache.compatibilityModeEnable) && (viewCache.viewPolicy != View.HMD_VIEW) && (viewCache.coexistenceCenteringEnable)
				&& (viewCache.coexistenceCenterInPworldPolicy == View.NOMINAL_SCREEN))
		{
			...
			{ // windowMovementPolicy == PHYSICAL_WORLD
	...
	private void computeCoexistenceToPlate()
	{
		...
		{
			coexistenceToLeftPlate.mul(headTrackerToLeftImagePlate, viewCache.trackerBaseToHeadTracker);
			coexistenceToLeftPlate.mul(viewCache.coexistenceToTrackerBase);

			if (useStereo)
			{
				coexistenceToRightPlate.mul(headTrackerToRightImagePlate, viewCache.trackerBaseToHeadTracker);
	...
	private void computeView(boolean doInfinite)
	{
		...
		vworldToCoexistenceScale = vworldToVpc.getDistanceScale() * vpcToCoexistence.getDistanceScale();
		if (doInfinite)
		{
			infVworldToCoexistenceScale = infVworldToVpc.getDistanceScale() * vpcToCoexistence.getDistanceScale();
		}

		if ((J3dDebug.devPhase) && (J3dDebug.canvasViewCache >= J3dDebug.LEVEL_2))
		{
			System.err.println("vworldToCoexistenceScale = " + vworldToCoexistenceScale);
		}
		...
		if (viewCache.compatibilityModeEnable)
		{
			...
			computeFrustumPlanes(leftProjection, leftVpcToEc, leftFrustumPlanes, leftFrustumPoints, leftCcToVworld);
		...
		if (viewCache.frontClipPolicy == View.VIRTUAL_EYE || viewCache.frontClipPolicy == View.VIRTUAL_SCREEN)
		{
		...
		if (viewCache.frontClipPolicy == View.PHYSICAL_EYE || viewCache.frontClipPolicy == View.VIRTUAL_EYE)
		{
			Fl = leftEyeInImagePlate.z + scale * -viewCache.frontClipDistance;
			Fr = rightEyeInImagePlate.z + scale * -viewCache.frontClipDistance;
		}
		else
		{
		...
		if (backClipPolicy == View.VIRTUAL_EYE || backClipPolicy == View.VIRTUAL_SCREEN)
		{
			scale = vworldToCoexistenceScale;
		}
		else
		...
		if (backClipPolicy == View.PHYSICAL_EYE || backClipPolicy == View.VIRTUAL_EYE)
		{
			// Yes, left for both left and right rear.
			B = leftEyeInImagePlate.z + scale * -backClipDistance;
		}
		else
		{
		...
		if (true /*viewCache.viewPolicy != View.HMD_VIEW*/)
		{
			...
			buildProjView(leftEyeInImagePlate, coexistenceToLeftPlate, vpcToLeftPlate, Fl, B, leftProjection, leftVpcToEc, false);
			...
			if (useStereo)
			{
				...
				buildProjView(rightEyeInImagePlate, coexistenceToRightPlate, vpcToRightPlate, Fr, B, rightProjection, rightVpcToEc, false);
			...
			if (doInfinite)
			{
				...
				buildProjView(leftEyeInImagePlate, coexistenceToLeftPlate, vpcToLeftPlate, leftEyeInImagePlate.z - 0.05,
						leftEyeInImagePlate.z - 1.5, infLeftProjection, infLeftVpcToEc, true);

				if (useStereo)
				{
					buildProjView(rightEyeInImagePlate, coexistenceToRightPlate, vpcToRightPlate, rightEyeInImagePlate.z - 0.05,
							rightEyeInImagePlate.z - 1.5, infRightProjection, infRightVpcToEc, true);
	 ...
	 */
	private void buildProjView(Point3d ep, Transform3D coe2Plate, Transform3D vpc2Plate, double F, double B, Transform3D ecToCc,
			Transform3D vpcToEc, boolean doInfinite)
	{

		// Lx,Ly Hx,Hy will be adjusted window boundaries
		double Lx, Hx, Ly, Hy;
		Lx = physicalWindowXLeft;
		Hx = physicalWindowXRight;
		Ly = physicalWindowYBottom;
	 ...
	 */
	private void computeFrustumPlanes(Transform3D ecToCc, Transform3D vpcToEc, Vector4d[] frustumPlanes, Point4d[] frustumPoints,
			Transform3D ccToVworld)
	{
		...
		// left
		computePlaneEq(frustumPoints[0], frustumPoints[4], frustumPoints[5], frustumPoints[1], frustumPlanes[0]);

		// right
		computePlaneEq(frustumPoints[3], frustumPoints[2], frustumPoints[6], frustumPoints[7], frustumPlanes[1]);

		// top
		computePlaneEq(frustumPoints[1], frustumPoints[5], frustumPoints[6], frustumPoints[2], frustumPlanes[2]);

		// bottom
		computePlaneEq(frustumPoints[0], frustumPoints[3], frustumPoints[7], frustumPoints[4], frustumPlanes[3]);

		// front
		computePlaneEq(frustumPoints[0], frustumPoints[1], frustumPoints[2], frustumPoints[3], frustumPlanes[4]);

		// back
		computePlaneEq(frustumPoints[4], frustumPoints[7], frustumPoints[6], frustumPoints[5], frustumPlanes[5]);
	...
	void getPixelLocationInImagePlate(double x, double y, double z, Point3d imagePlatePoint)
	{
		...
		if ((viewCache.projectionPolicy == View.PERSPECTIVE_PROJECTION) && (centerEyeInImagePlate.z != 0))
		{
			double zScale = 1.0 - z / centerEyeInImagePlate.z;
			imagePlatePoint.x = (screenx - centerEyeInImagePlate.x) * zScale + centerEyeInImagePlate.x;
			imagePlatePoint.y = (screeny - centerEyeInImagePlate.y) * zScale + centerEyeInImagePlate.y;
		}
		else
		{
	...
	void getPixelLocationFromImagePlate(Point3d imagePlatePoint, Point2d pixelLocation)
	{
		...
		pixelLocation.y = screenViewCache.screenHeight - 1 - (screenY / screenViewCache.metersPerPixelY) - canvasY;
