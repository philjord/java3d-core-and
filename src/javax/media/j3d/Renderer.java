...
class Renderer extends J3dThread
{
	...
	void doWork(long referenceTime)
	{
		...
		double accumLeftX = 0.0, accumLeftY = 0.0, accumRightX = 0.0, accumRightY = 0.0, accumInfLeftX = 0.0, accumInfLeftY = 0.0,
				...
				for (i = 0; i < swapArray.length; i++)
				{
					...
					doneSwap: try
					{
						...
						if (cv.active && (cv.ctx != null) && (cv.view != null) && (cv.imageReady))
						{
							...
							if (!cv.ctxChanged)
							{
								cv.canvasDirty = (0xffff
										& ~(Canvas3D.LIGHTBIN_DIRTY | Canvas3D.LIGHTENABLES_DIRTY | Canvas3D.AMBIENTLIGHT_DIRTY
												| Canvas3D.MODELCLIP_DIRTY | Canvas3D.VIEW_MATRIX_DIRTY | Canvas3D.FOG_DIRTY));
					...
					catch (RuntimeException ex)
					{
						...
						cv.setFatalError();
						RenderingError err = new RenderingError(RenderingError.UNEXPECTED_RENDERING_ERROR, J3dI18N.getString("Renderer0"));
						err.setCanvas3D(cv);
					...
					else if (renderType == J3dMessage.RENDER_IMMEDIATE)
					{
						...
						try
						{
							...
							case GraphicsContext3D.SET_LIGHT:
								canvas.graphicsContext3D.doSetLight((Light) m[nmesg].args[2], ((Integer) m[nmesg].args[3]).intValue());
								break;
							case GraphicsContext3D.INSERT_LIGHT:
								canvas.graphicsContext3D.doInsertLight((Light) m[nmesg].args[2], ((Integer) m[nmesg].args[3]).intValue());
								break;
							...
							case GraphicsContext3D.SET_SOUND:
								canvas.graphicsContext3D.doSetSound((Sound) m[nmesg].args[2], ((Integer) m[nmesg].args[3]).intValue());
								break;
							case GraphicsContext3D.INSERT_SOUND:
								canvas.graphicsContext3D.doInsertSound((Sound) m[nmesg].args[2], ((Integer) m[nmesg].args[3]).intValue());
								break;
							...
							case GraphicsContext3D.SET_BUFFER_OVERRIDE:
								canvas.graphicsContext3D.doSetBufferOverride(((Boolean) m[nmesg].args[2]).booleanValue());
								break;
							case GraphicsContext3D.SET_FRONT_BUFFER_RENDERING:
								canvas.graphicsContext3D.doSetFrontBufferRendering(((Boolean) m[nmesg].args[2]).booleanValue());
								break;
					...
					{ // retained mode rendering
						...
						if (canvas.ctx == null)
						{
							...
							synchronized (VirtualUniverse.mc.contextCreationLock)
							{
								...
								for (NodeComponentRetained nc : renderBin.nodeComponentList)
								{
						...
						if (renderBin != null)
						{
							...
							if (canvas.useSharedCtx)
							{
								...
								if (dirtyDisplayList)
								{
									renderBin.updateDirtyDisplayLists(canvas, dirtyRenderMoleculeList, dirtyDlistPerRinfoList,
							...
							else
							{
								...
								{
									renderBin.updateDirtyDisplayLists(canvas, canvas.dirtyRenderMoleculeList, canvas.dirtyDlistPerRinfoList,
							...
							if (canvas.view.getSceneAntialiasingEnable() && canvas.sceneAntialiasingAvailable)
							{
										...
										|| !canvas.sceneAntialiasingMultiSamplesAvailable)
								{
									...
									if (useStereo)
									{
										System.arraycopy(cvCache.getRightProjection().mat, 0, accumRightProj.mat, 0, 16);
									...
									{
										System.arraycopy(cvCache.getInfLeftProjection().mat, 0, accumInfLeftProj.mat, 0, 16);
										accumInfLeftX = accumInfLeftProj.mat[3];
										accumInfLeftY = accumInfLeftProj.mat[7];
										if (useStereo)
										{
											System.arraycopy(cvCache.getInfRightProjection().mat, 0, accumInfRightProj.mat, 0, 16);
							...
							for (pass = 0; pass < num_stereo_passes; pass++)
							{
								...
								for (apass = 0; apass < num_accum_passes; apass++)
								{
									...
									if (doAccum)
									{
										...
										accumLeftProj.mat[3] = accumLeftX + accumLeftProj.mat[0] * accumDx + accumLeftProj.mat[1] * accumDy;

										accumLeftProj.mat[7] = accumLeftY + accumLeftProj.mat[4] * accumDx + accumLeftProj.mat[5] * accumDy;
										...
										if (renderBin.geometryBackground != null)
										{
											accumInfLeftProj.mat[3] = accumInfLeftX + accumInfLeftProj.mat[0] * accumDx
													+ accumInfLeftProj.mat[1] * accumDy;

											accumInfLeftProj.mat[7] = accumInfLeftY + accumInfLeftProj.mat[4] * accumDx
													+ accumInfLeftProj.mat[5] * accumDy;

											if (useStereo)
											{
												accumInfRightProj.mat[3] = accumInfRightX + accumInfRightProj.mat[0] * accumDx
														+ accumInfRightProj.mat[1] * accumDy;

												accumInfRightProj.mat[7] = accumInfRightY + accumInfRightProj.mat[4] * accumDx
